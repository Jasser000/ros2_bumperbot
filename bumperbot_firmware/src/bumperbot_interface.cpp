...
namespace bumperbot_firmware
{
    ...
    BumperbotInterface::~BumperbotInterface()
    {
        if(arduino_.IsOpen())
        {
            ...
            {
                RCLCPP_FATAL_STREAM(rclcpp::get_logger("BumperbotInterface"), "Somthing went wrong while closing the connection with port " << port_);
            }
    ...
    CallbackReturn BumperbotInterface::on_init(const hardware_interface::HardwareInfo & hardware_info)
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("BumperbotInterface"), "No Serial Port Provided! Aborting");
    ...
    CallbackReturn BumperbotInterface::on_activate(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("BumperbotInterface"), "Somthing went wrong while interacting with port " << port_);
            return CallbackReturn::FAILURE;
        }
    ...
    CallbackReturn BumperbotInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        if(arduino_.IsOpen())
        {
            ...
            {
                RCLCPP_FATAL_STREAM(rclcpp::get_logger("BumperbotInterface"), "Somthing went wrong while closing the port " << port_);
                return CallbackReturn::FAILURE;
            }
        }
    }
    ...
    hardware_interface::return_type BumperbotInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period)
    {
        ...
        {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("BumperbotInterfarce"), "Somthing went wrong while sending the message " <<
